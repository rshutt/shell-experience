...
function __promptline_kubernetes {
  ...
  if [ -x "$(command -v kubectl)" ]; then
    ...
    ns=$(kubectl config view --minify --output 'jsonpath={.contexts[0].context.namespace}')
...
function __promptline_git_status {
  ...
  [[ $(git rev-parse --is-inside-work-tree 2>/dev/null) == true ]] || { printf "%s" "$leading_whitespace$clean_symbol"; leading_whitespace=" "; return 0 }
...
function __promptline {
  ...
  if [[ -n ${ZSH_VERSION-} ]]; then
    left="$(__promptline_left_prompt)"
    right="$(__promptline_right_prompt)"
    fill-line "${left}" "${right}"
    PROMPT="${REPLY}${newline}${a_bg}${space}${reset}${a_sep_fg}${sep}${reset}${space}"
    export PROMPT
  elif [[ -n ${FISH_VERSION-} ]]; then
    if [[ -n "$1" ]]; then
